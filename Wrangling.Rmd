...
avg_wpd_dataset <- wpd_dataset %>%
  group_by(yearID) %>%
  summarize(AverageWinsPerDollar = mean(Wpd))
...
```{r}
Calculations2 <- mutate(calculations, Expected_Wins = Payroll * AverageWinsPerDollar)
Calculations2
```
...
```{r}
Calculations3 <- mutate(Calculations2, residual_wins = W - Expected_Wins)
Calculations3
```
